...
public static partial class McpMod
{
    private static Dictionary<string, object?> BuildGameState()
    {
        ...
        var rawOverlay = NOverlayStack.Instance?.Peek();
        var currentRoom = runState.CurrentRoom;
        bool mapIsOpen = IsMapScreenOpenOrVisible();
        var topOverlay = ForegroundOverlay(rawOverlay, rawOverlay?.GetType(), mapIsOpen);
        ...
        {
            // ForegroundOverlay already removed reward screens lingering beneath the map.
        ...
        if (currentRoom is CombatRoom selectionCombat)
            AddSelectionBattleContext(result, CombatManager.Instance.IsInProgress, () => BuildBattleState(runState, selectionCombat));
    ...
    private static void AddSelectionBattleContext(Dictionary<string, object?> state, bool combat, Func<Dictionary<string, object?>> buildBattle)
    {
        if (combat && state["state_type"] is "card_select" or "card_reward")
            state["battle"] = buildBattle(); // Existing observed-node and readable-intent filters remain authoritative.
    }
    ...
    private static Dictionary<string, object?> BuildStartRunLobbyState(StartRunLobby lobby)
    {
        var lobbyState = new Dictionary<string, object?>
        {
            ...
            // v0.111 keeps the lobby capacity in a private field.
            ["max_players"] = GetInstanceFieldValue(lobby, "_maxPlayers"),
    ...
    private static Dictionary<string, object?> BuildBattleState(RunState runState, CombatRoom combatRoom)
    {
        ...
        {
            if (creature.IsAlive && visible.Contains(creature))
            {
    ...
    private static Dictionary<string, object?> BuildPlayerState(Player player)
    {
        ...
        if (combatState != null && CombatManager.Instance.IsInProgress)
        {
            ...
            // Full visible-card tie-breaker: same-ID upgrades/cost changes must not reveal draw order.
            state["draw_pile"] = BuildPileCardList(drawCards, PileType.Draw)
                .OrderBy(c => System.Text.Json.JsonSerializer.Serialize(c), StringComparer.Ordinal).ToList();
        ...
        state["deck"] = player.Deck.Cards.Select(card => BuildCardInfo(card, PileType.Deck)).ToList();
    ...
    private static Dictionary<string, object?> BuildEnemyState(Creature creature, Dictionary<string, int> entityCounts)
    {
        ...
        if (monster?.IsHealthBarVisible != true)
        {
            state.Remove("hp");
            state.Remove("max_hp");
            state.Remove("block");
        ...
        // Read only intent models already attached to visible UI, never future move-state data.
        var creatureNode = FindAll<NCreature>(((SceneTree)Engine.GetMainLoop()).Root)
            .FirstOrDefault(n => n.Entity == creature);
        if (creatureNode?.IntentContainer is { } intentContainer && IsReadableCanvas(intentContainer))
        {
            var intents = new List<Dictionary<string, object?>>();
            foreach (var node in FindAll<NIntent>(intentContainer).Where(IsReadableCanvas))
            {
                if (GetInstanceFieldValue(node, "_intent") is not AbstractIntent intent)
                    throw new InvalidOperationException("Visible intent model unavailable");
                if (intent is HiddenIntent) continue;
                intents.Add(BuildVisibleIntent(intent, creature, node));
            }
    ...
    private static Dictionary<string, object?> BuildVisibleIntent(AbstractIntent intent, Creature creature, NIntent node)
    {
        if (GetInstanceFieldValue(node, "_targets") is not IEnumerable<Creature> targets)
            throw new InvalidOperationException("Visible intent targets unavailable");
        var hoverTip = intent.GetHoverTip(targets, creature);
        return new()
        {
            ["type"] = intent.IntentType.ToString(),
            ["label"] = ReadVisibleText(node),
            ["title"] = hoverTip.Title == null ? null : StripRichTextTags(hoverTip.Title),
            ["description"] = hoverTip.Description == null ? null : StripRichTextTags(hoverTip.Description)
        };
    }
    ...
    private static Dictionary<string, object?> BuildEventState(EventRoom eventRoom, RunState runState)
    {
        ...
        // During dialogue, options/future lines are not strategic context even though their nodes exist.
        if (inDialogue && FindFirst<NAncientEventLayout>(uiRoom!) is { } dialogueLayout)
        {
            var container = GetInstanceFieldValue(dialogueLayout, "_dialogueContainer") as Node
                ?? throw new NotSupportedException("event_dialogue_context_unavailable");
            int line = (int)(GetInstanceFieldValue(dialogueLayout, "_currentDialogueLine")
                ?? throw new NotSupportedException("event_dialogue_line_unavailable"));
            state["body"] = string.Join("\n", container.GetChildren().Take(line + 1).Select(ReadVisibleText));
        }
        else state["body"] = uiRoom == null ? null : ReadVisibleText(uiRoom);

        // Options from the current layout page only, keyed by the same native index dispatch uses; dialogue has a separate native input surface.
        var options = new List<Dictionary<string, object?>>();
        if (uiRoom?.Layout is { } layout && !inDialogue)
        {
            foreach (var button in layout.OptionButtons)
            {
                if (!IsReadableCanvas(button)) continue;
                var opt = button.Option;
                var optData = new Dictionary<string, object?>
                {
                    ["index"] = EventOptionIndex(button),
    ...
    private static Dictionary<string, object?> BuildFakeMerchantState(EventRoom eventRoom, RunState runState)
    {
        ...
        bool isOpen = fakeMerchantNode != null && FindFirst<NMerchantInventory>(fakeMerchantNode)?.IsOpen == true;
        state["is_open"] = isOpen;
        // Do not reveal unseen inventory before an explicit open action.
        var shopState = isOpen ? BuildFakeMerchantShopItems(fakeMerchant.Inventory)
    ...
    private static Dictionary<string, object?> BuildShopState(MerchantRoom merchantRoom, RunState runState)
    {
        ...
        state["is_open"] = NMerchantRoom.Instance?.Inventory?.IsOpen == true;
        if (state["is_open"] is not true)
            return state;
    ...
    private static Dictionary<string, object?> BuildMapState(RunState runState)
    {
        ...
        if (mapScreen != null)
        {
            // Only rendered map nodes/paths, not ActMap's hidden graph or boss identity metadata.
            var visiblePoints = FindAll<NMapPoint>(mapScreen)
                .Where(mp => mp.Point != null && IsReadableCanvas(mp)).ToList();
            var visibleCoords = visiblePoints.Select(mp => mp.Point.coord).ToHashSet();
            if (GetInstanceFieldValue(mapScreen, "_paths") is not
                Dictionary<(MapCoord, MapCoord), IReadOnlyList<TextureRect>> paths)
                throw new NotSupportedException("visible_map_paths_unavailable");
            var edges = paths.Where(p => visibleCoords.Contains(p.Key.Item1)
                && visibleCoords.Contains(p.Key.Item2) && p.Value.Any(IsReadableCanvas)).Select(p => p.Key).ToList();
            state["nodes"] = visiblePoints.OrderBy(mp => mp.Point.coord.row).ThenBy(mp => mp.Point.coord.col)
                .Select(mp => new Dictionary<string, object?>
                {
                    ["col"] = mp.Point.coord.col,
                    ["row"] = mp.Point.coord.row,
                    ["type"] = mp.Point.PointType.ToString(),
                    ["children"] = edges.Where(e => e.Item1.Equals(mp.Point.coord))
                        .OrderBy(e => e.Item2.col).ThenBy(e => e.Item2.row)
                        .Select(e => new[] { e.Item2.col, e.Item2.row }).ToList()
                }).ToList();
            var travelable = visiblePoints
    ...
    private static Dictionary<string, object?> BuildRewardsState(NRewardsScreen rewardsScreen, RunState runState)
    {
        ...
        {
            var button = rewardButtons[index];
            if (button.Reward == null || button.Reward.SuccessfullySelected || !IsControlVisibleOrActionable(button)) continue;
    ...
    private static List<Dictionary<string, object?>> BuildPowersState(Creature creature)
    {
        ...
        foreach (var power in creature.Powers)
        {
            ...
            // An unreadable visible power makes the observation unsafe, not a smaller legal state.
            try
            {
            ...
            catch (Exception e) { throw new InvalidOperationException("Visible power text unavailable", e); }
        }
        return powers;
    }
