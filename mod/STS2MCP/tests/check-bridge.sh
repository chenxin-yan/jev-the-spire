...
if (args.Length > 1)
{
    ...
    // ActionQueueChanged restarted the idle executor, or the still-running batch's own Task.
    void BindReceipt(object operation, object source, Task batch, bool resumedAfterPlayerChoice)
        => Call(exitType, operation, "BindExecutionReceipt", source, batch, resumedAfterPlayerChoice);
    ...
    // Opcodes plus resolved method, field, type (typeof/isinst/castclass) and string operands of one compiled method body.
    ...
    // Pin the comparison immediately passed to binding, not merely a State read elsewhere in the phase guard.
    var executingCallback = bridge.GetNestedTypes(flags).SelectMany(t => t.GetMethods(flags | BindingFlags.DeclaredOnly))
        .Single(m => m.Name.Contains("<RegisterCombatExit>g__Executing|"));
    var callbackBody = Operands(executingCallback);
    int bindIndex = callbackBody.FindIndex(o => o is MethodBase m && m.Name == "BindExecutionReceipt" && m.DeclaringType == exitType);
    Check(bindIndex >= 6 && callbackBody[bindIndex - 6].Equals(OpCodes.Ldarg_1)
        && callbackBody[bindIndex - 5].Equals(OpCodes.Callvirt)
        && callbackBody[bindIndex - 4] is MethodBase stateGetter && stateGetter.Name == "get_State" && stateGetter.DeclaringType == nativeGameAction
        && callbackBody[bindIndex - 3].Equals(OpCodes.Ldc_I4_4) && callbackBody[bindIndex - 2].Equals(OpCodes.Ceq)
        && callbackBody[bindIndex - 1].Equals(OpCodes.Callvirt),
        "compiled callback forwards source.State == ReadyToResumeExecuting directly to BindExecutionReceipt, not a constant flag");
