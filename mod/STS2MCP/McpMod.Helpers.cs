...
public static partial class McpMod
{
    ...
    {
        // Base Description is not a safe fallback for a failed dynamic/pile description.
        return BridgeProtocol.RequiredText(() => StripRichTextTags(card.GetDescriptionForPile(pile)));
    }
        ...
        try
        {
            foreach (var tip in IHoverTip.RemoveDupes(tips))
            {
                ...
                if (tip is HoverTip ht)
                {
                    ...
                    description = BridgeProtocol.RequiredText(() => StripRichTextTags(ht.Description));
                }
                else if (tip is CardHoverTip cardTip)
                {
                    title = SafeGetText(() => cardTip.Card.Title);
                    description = BridgeProtocol.RequiredText(() => SafeGetCardDescription(cardTip.Card));
                }
                else throw new NotSupportedException("unsupported_rules_hover_tip");
                result.Add(new() { ["name"] = title, ["description"] = description });
            }
        }
        catch (Exception e) { throw new NotSupportedException("required_hover_tips_unavailable", e); }
        return result;
    }
