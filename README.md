...
Exit status (Crust's native contract): 0 on `terminal` or `max_actions`; 130 on Ctrl-C (the aborted summary is still logged and printed, then the Effect is interrupted, which Crust reports silently); 1 on `halted` (`Error: <outcome>: <halt_reason>` on stderr after the summary) and on bad arguments. Crust owns the SIGINT listener: the first Ctrl-C aborts the invocation `signal` (reported verbatim as `halt_reason` `Interrupted by SIGINT.`), which the loop honors through pending I/O and inference; the loop runs uninterruptible so the summary is written before Crust cleanup and exit. A second Ctrl-C force-quits.
